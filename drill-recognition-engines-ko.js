 ...
 *
 * English-localized sibling of drill-recognition-engines-ja.js. Loaded as a
...
(function () {
  ...
  // whisper-small is markedly more accurate on Korean than -base, which matters
  ...
  // deliver(text)        — finalize with a transcript (empty => "no speech").
  // deliver('', errorMsg) — finalize after an error; keep the error message visible.
  ...
  function startCapture() {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      deliver('', 'This browser cannot access the microphone.'); return;
    }
    navigator.mediaDevices.getUserMedia({
      ...
      deliver('', 'Could not access the microphone: ' + ((err && err.message) || err));
    });
  }
  ...
  function finishCapture() {
    ...
    resampleTo16k(merged, rate).then(function (a16) {
      ...
      deliver('', 'Audio processing failed.');
    });
  }
  ...
  function whisperTranscribe(a16) {
    var w = ensureWhisper();
    if (!w) { deliver('', 'Could not start the on-device speech engine.'); return; }
    w.curId = ++whisperJob;
    w.postMessage({
      type: 'transcribe', id: w.curId, audio: a16,
      model: WHISPER_MODEL, device: 'wasm', language: 'korean',
      // Tuned for short Korean utterances: greedy+low temp for stable output,
  ...
  function onWhisperMsg(e) {
    ...
    if (m.type === 'transcript') {
      ...
      else { deliver('', 'Transcription failed: ' + m.error); }
    }
  }
  ...
  function elevenlabsTranscribe(a16) {
    ...
    function serverTranscribe() {
      fetch('/.netlify/functions/stt', {
        ...
        body: JSON.stringify({ audio_base64: bufToBase64(wav), language: 'ko' })
  ...
  function cloudTranscribe(a16) {
    ...
    if (!key) {
      deliver('', 'Cloud transcription needs an OpenAI API key — enter it in the Recognition settings.');
      return;
    }
    var form = new FormData();
    ...
    fetch('https://api.openai.com/v1/audio/transcriptions', {
      ...
      deliver('', 'Cloud transcription failed: ' + ((err && err.message) || err));
    });
  }

  // ── the recognizer shim (drop-in for the Web Speech object) ────────────────
  var shim = {
    lang: 'ko-KR', continuous: false, interimResults: true, maxAlternatives: 1,
    onresult: null, onend: null, onerror: null, onstart: null,
    start: function () {
      if (state.engine === 'browser') {
        if (browserRecog) { browserRecog.start(); }
        else { deliver('', 'Browser speech recognition is not available. Pick the Whisper engine.'); }
      } else {
        startCapture();
      }
    },
